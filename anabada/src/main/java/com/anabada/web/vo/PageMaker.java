...
public class PageMaker {
	...
	public String makeSearch(int page) { // list.jsp에서 페이지 숫자 눌렀을 때 실행되는 메소드
		UriComponents uriComponents = 
				...
				.queryParam("addr1", encoding(((SearchCriteria)cri).getJ_addr1()))
				.queryParam("term", encoding(((SearchCriteria)cri).getJ_term()))
				.queryParam("method", encoding(((SearchCriteria)cri).getJ_method()))
				.queryParam("cate", encoding(((SearchCriteria)cri).getJ_cate()))
				.build();
